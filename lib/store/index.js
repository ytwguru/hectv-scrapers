...
const updatePost = (data) => new Promise((resolve) => db.wpPosts.update({
  post_author: 0,
  post_title: data.name,
  post_content: data.description ? data.description : '',
  post_excerpt: data.description ? data.description.substring(0, 100) : '',
...
const createOrUpdatePostmetaHelper = (postId, event) => Promise.all([
  createOrUpdatePostmeta(postId, 'venue', event.location ? event.location : ''),
  createOrUpdatePostmeta(postId, 'event_price', event.price ? event.price : ''),
  ...
  if (event.date) {
    try {
      ...
      if (startDate && endDate) {
        ...
        await cuEvent(newPost.id, 'event_dates', 1,
  ...
  db.sequelize.authenticate().then(() => {
    try {
      createSites(data)
        .then(async ([site]) => {
          try {
            const response = await Promise.all(data.events.map((e) => e && createEvents(e, site.id)
