  ...
  createPostmeta(id, key, value)
    .then(([newPostmeta, created]) => {
      if (!created) {
        ...
        console.log(`Creating NEW: key - ${key}. value: ${value}`);
        resolve(newPostmeta);
      }
    })
    .catch((err) => {
  ...
  findLikePostmeta(id, '_end_time', endDate).then(async (postmeta) => {
    if (postmeta && !updateTimes) {
      ...
      try {
        ...
        if (!hasEvent.created) {
          await cuEvent(id, key, value + 1, startDate, endDate, false);
        }
        resolve(postmeta);
      } catch (err) {
...
const createOrUpdatePostHelper = async (event, newPost, updating) => {
  if (event.date) {
    try {
      ...
      if (startDate && endDate) {
        ...
        if (updating) {
          console.log(
            `==Updating events for ${newPost.id}== \nNew Start - ${startDate}. New End - ${endDate}.`,
          );
          updateTimes = newPost.post_status !== 'publish'; // update the event dates if the user hasn't published the event
        }
        await cuEvent(
          newPost.id,
          'event_dates',
  ...
  if (created) {
    ...
    updatePost(event)
      .then(async (updatedPost) => {
        console.log(
          `Updating Postmeta - Event id: ${event.id}. Post id: ${updatedPost.id}`,
        );
        await createOrUpdatePostHelper(event, updatedPost, true);
        resolve(updatedPost);
      })
      .catch((err) => {
