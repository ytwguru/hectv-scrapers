...
const scrape = async ({ limit, currentPage = '2018-08-15' }) => {
  ...
  const pageDate = dayjs(currentPage, 'YYYY-MM-DD').format('YYYY/MM/DD');
  const userAgent = new UserAgent();
  const options = {
    method: 'get',
    uri: `https://do314.com/events/${pageDate}`,
    headers: {
    ...
    $listings.toArray().map(
      (e) => new Promise(async (resolve) => {
        try {
          ...
          const startDateTxt = clean(
            $(
              '.ds-event-detail > .ds-event-detail-main .ds-event-meta-container .ds-event-time > span',
            ).attr('data-datetime'),
          );
          const endDateTxt = clean(
            $(
              '.ds-event-detail > .ds-event-detail-main .ds-event-meta-container .ds-event-time',
            )
              .find('span')
              .last()
              .attr('data-datetime'),
          );
          const displayDateTxt = clean(
            $(
              '.ds-event-detail > .ds-event-detail-main .ds-event-meta-container .ds-event-time > span',
            ).attr('data-datetime'),
          );

          const date = moment(new Date(displayDateTxt)).format(
            'YYYY-MM-DD HH:mm:ss',
          );
          const startDate = moment(
            new Date(startDateTxt || displayDateTxt),
          ).format('YYYY-MM-DD HH:mm:ss');
          const endDate = moment(
            new Date(endDateTxt || displayDateTxt),
          ).format('YYYY-MM-DD HH:mm:ss');
